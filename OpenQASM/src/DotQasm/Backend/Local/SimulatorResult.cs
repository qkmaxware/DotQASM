...
public class SimulatorResult: BackendResult {
    ...
    public IEnumerable<Complex> StateVector {get; private set;}
    ...
    public SimulatorResult(IBackend backend, TimeSpan time, IEnumerable<Complex> state, IEnumerable<State> register) {
        ...
        this.StateProbabilityHistogram = this.StateVector.Select((amplitude, state) => new KeyValuePair<int, double>(
